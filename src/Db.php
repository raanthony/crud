...
class Db
{
    ...
    public static function getSearchInputStr ( $field )
    {
        ...
        if ( $field->type == 'enum' )
        {
            $output = "{!!\Idb\Crud\Html::selectRequested(\n";
            $output .= "\t\t\t\t\t'".$field->name."',\n";
    ...
    public static function getFormInputMarkup ( $field, $modelName = '' )
    {
        ...
        if ( $field->type == 'enum' )
        {
            return "{!! \Idb\Crud\Form::select( '{$field->name}', [ '".join("', '",$field->enumValues)."' ] ){$modelStr}->show() !!}";
        }

        if ( $field->type == 'text' )
        {
            return "{!! \Idb\Crud\Form::textarea( '{$field->name}' ){$modelStr}->show() !!}";
        }

        // input type:
        $type = 'text';
        if ( $field->type == 'date' ) $type = $field->type;
        return "{!! \Idb\Crud\Form::input('{$field->name}','{$type}'){$modelStr}->show() !!}";
    }
